...
const DEFAULT_WAVE_MS := 550
const DEFAULT_HOLD_MS := 600
const DEFAULT_ZOOM_IN_MS := 350
...
var impact_beat_zoom_in_ms: int = DEFAULT_ZOOM_IN_MS
...
func apply(config: Variant) -> void:
	...
	impact_beat_zoom_in_ms = maxi(0, int(hooks.get("impactBeatZoomInMs", DEFAULT_ZOOM_IN_MS)))
...
func impact_beat_total_ms() -> int:
	return (
		impact_beat_zoom_out_ms +
		impact_beat_wave_ms +
		impact_beat_hold_ms +
		impact_beat_zoom_in_ms
	)
