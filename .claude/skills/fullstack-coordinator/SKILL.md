...
the policy. This skill pins the contract between them and hands off.

**Route:** [`networking.md`](../../../docs/context/networking.md) § only. Do not
load `backend.md` or `ui.md` here — the role skill you hand to will load its own.
...
3. **Hand off in sequence** — `server-engineer`, then `client-engineer` — and
   carry the pinned contract into each. Do not edit both sides yourself from
   here; the role policies are what keep each side in shape.
