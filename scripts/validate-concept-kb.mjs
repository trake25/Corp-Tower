...
function parseArgs(argv) {
  ...
  return { root: resolve(positionals[0] || '.'), kbRoot, quiet, mapPaths };
}
...
function conceptMapFile(expected, concept) {
  return resolve(expected.kb_root, 'docs/context/map/concept', `${concept.domain}.md`);
}

export function validateConceptKb({ root = '.', kbRoot = 'KB', mapPaths = [] } = {}) {
  ...
  if (existsSync(proseRoot)) {
    const documents = readdirSync(proseRoot, { withFileTypes: true })
      ...
      .filter(file => !selected || file !== join(proseRoot, 'index.md'));
    const mapsRoot = join(proseRoot, 'map/concept');
    const allMaps = existsSync(mapsRoot)
      ? readdirSync(mapsRoot).filter(name => name.endsWith('.md')).map(name => join(mapsRoot, name))
      : [];
    const maps = selected ? allMaps.filter(file => selected.has(resolve(file))) : allMaps;
  ...
  const expectedMaps = new Set(
    [...expected.outputs.keys()].filter(file => file.replaceAll('\\', '/').includes('/map/concept/')),
  );

  if (selected) {
    for (const file of selected) {
      if (!expectedMaps.has(file))
        errors.push({ status: 'map-stale', message: `selected concept map is not generated by the current registry: ${relative(expected.root, file)}` });
    }
  }

  for (const [file, body] of expected.outputs) {
    const isConceptMap = expectedMaps.has(file);
    if (selected && (!isConceptMap || !selected.has(file))) continue;
  ...
  if (selected) {
    for (const file of selected) {
      if (!existsSync(file) || !expectedMaps.has(file)) continue;
      if (!readFileSync(file, 'utf8').includes(CONCEPT_MAP_MARKER))
        errors.push({ status: 'map-stale', message: `concept map lacks generated marker: ${relative(expected.root, file)}` });
    }
  } else if (existsSync(mapRoot)) {
  ...
  const mapsForSections = selected
    ? [...expectedMaps].filter(file => selected.has(file))
    : [...expectedMaps];
  const mappedIds = new Set();
  for (const file of mapsForSections) {
  ...
  for (const concept of expected.concepts) {
    if (selected && !selected.has(conceptMapFile(expected, concept))) continue;
  ...
  return {
    ...
    sources: expected.concepts.reduce((sum, concept) => sum + concept.sources.length, 0),
    errors,
    warnings,
  };
}
