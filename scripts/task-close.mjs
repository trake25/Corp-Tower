...
export function createManifest({ task, changedPaths, runId = null }) {
...
function verify(manifest, manifestFile) {
  ...
  const receipt = { schema_version: SCHEMA_VERSION, task: manifest.task, manifest: displayPath(manifestFile), status: failed ? 'failed' : 'passed', steps };
  writeFileSync(receiptPath(manifestFile), `${JSON.stringify(receipt, null, 2)}\n`);
  manifest.verification = { status: receipt.status, receipt: displayPath(receiptPath(manifestFile)) };
...
async function main() {
  ...
  fail('usage: node scripts/task-close.mjs <prepare|decide|verify> ...');
}
