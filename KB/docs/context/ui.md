...
source: src/Client/App/corp-tower/Cor/Scripts/JoinScreen.gd#_refresh_password_presentation
adjacent: network.room.private
-->
## Join Server
...
source: src/Client/App/corp-tower/Cor/Scripts/ScreenManager.gd#update_debug_button_availability
adjacent: backend.lobby.debug-config
adjacent: network.room.bot-spectator
-->
## Debug entry

The floating debug entry is build-gated and context-sensitive. On Home it opens a compact Bot Spectator Match setup with mixed/single-personality presets, three independent personality selectors, and per-bot reaction, skill, risk, greed, repair, and Power traits. During connected Play, including Bot Spectator Play, the existing Debug tuning panel is available while recovery or shell-level input blocking is inactive. Spectator debug tuning does not grant gameplay participation. Drag position persists within the intended app/room lifecycle, and client gating never replaces server authorization.
