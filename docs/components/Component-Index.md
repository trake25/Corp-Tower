# Component Index

## Server
- [[Server Entry]]
- [[Lobby Manager]]
- [[Redis State]]
- [[Game Engine]]
- [[Block Supply]]
- [[Scoring]]
- [[Impacts]]
- [[Tower Stability]]
- [[Bot Manager]]
- [[Game Config]]
- [[Server Docker Image]]
- [[Balance Simulator]]
- [[Server Score Events Tests]]

## Client
- [[Godot Client App]]
- [[NetworkManager]]
- [[Screen Manager]]
- [[Main UI Controller]] (slim orchestrator + the `Cor/Scripts/GameUi/` module
  family: UiTuning, MatchState, PlayerContext, UiNodeBinder, PointerEvents,
  PopoverCoordinator, BlockData, DebugPanelController, ScorePopupController,
  LevelSummaryController, RosterViewController, QuestController,
  QuickChatController, PowerController, InventoryController, TopBarController
  — documented within Main UI Controller, not as separate files)
- [[Game UI Scene]]
- [[Block Preview]]
- [[Tower Stack]]
- [[Popover Panel]]
- [[Impact Bar]]
- [[Cooldown Overlay]]
- [[Debug Overlay]]
- [[Player Colors]]
- [[Godot Client Tests]]

## CI/CD And Infrastructure
- [[Client Android Internal Workflow]]
- [[Client HTML5 Pages]]
- [[Private Asset Pipeline]]
- [[Terraform Infrastructure]]
- [[Server K3s Stack]]
- [[Server K3s Workflows]]
- [[Server K3s Automated Master Workflow]]

## Global
- [[Summary]]
