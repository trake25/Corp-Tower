...
`scripts/write-endpoint-config.sh` regenerates the committed `src/Client/App/corp-tower/Sys/NetMan/Endpoint_Config.gd` before each client build, from three env vars: `CORP_TOWER_WS_PRIMARY` (required), `CORP_TOWER_WS_FAILOVER` (optional — empty disables client-side failover, see [networking.md § Connection](./networking.md#connection)), `CORP_TOWER_DEBUG_UI` (`true`/`false`, gates the floating debug button — see [ui.md](./ui.md#screen-manager)). The committed default (dev instance 1 primary, dev instance 2 failover, debug on) is what a local `godot --editor` run or an un-rewritten build gets. Every CI build that ships a real endpoint calls this script first: [Android Deploy wsplaytod](#android-deploy-wsplaytod-workflow), [Client HTML5 Pages](#client-html5-pages) below, and the backup's `Backup-Deploy-Web-Server.yml` per instance ([deployment.md § Backup](./deployment.md#backup-physical-machine-4-dev-instances)).

## Client HTML5 Pages

`.github/workflows/Client-HTML5-Pages.yml` (+ paired `Client-HTML5-Undeploy.yml`) — builds/exports/deploys the Godot client as WebGL to GitHub Pages. Runs on `ubuntu-24.04`; triggers on push to `main`/`master` for `src/Client/**`, the `build-godot-web`/`fetch-private-assets` composite actions, `.github/godot/export_presets.web.ci.cfg`, and its own workflow file, or manual `workflow_dispatch`/`workflow_call`. Input: `art_version_override` (testing only).

**Behavior:** calls `actions/configure-pages` with `enablement: true` (so a torn-down Pages site is recreated on next deploy) → fetches private art via `fetch-private-assets` (fails closed on missing/bad-hash/incomplete bundle) → writes the client endpoint config (primary `wsplaytod.galaxxigames.com`, no failover, debug UI **off** — see [Client endpoint config](#client-endpoint-config) above) → builds via the `build-godot-web` composite action (downloads Godot `4.6.2.stable` + matching Web export templates, applies `.github/godot/export_presets.web.ci.cfg`, imports headless, exports a Web release, writes a file-size report to the job summary) → disables Jekyll (`.nojekyll`) → writes a `CNAME` file (`play.tod.galaxxigames.com`, inert — see Custom domain below) → uploads `build/web` as a Pages artifact and deploys to the `github-pages` environment → removes fetched art (`if: always()`).

**Auto-deploy guard rail (`push` trigger only):** a `check-pages-status` job runs before `build` and `curl`s `https://play.tod.galaxxigames.com/` — if it gets a `404` (hard-undeployed) or the soft-undeploy offline placeholder body, `build`/`deploy` are skipped so a routine client push doesn't silently undo an intentional `Client-HTML5-Undeploy.yml` run. Only gates `push`; manual `workflow_dispatch`/`workflow_call` always run — `deploy` carries an explicit `if: always() && needs.build.result == 'success'` so it isn't itself skipped by `check-pages-status`'s skip on manual runs. Full rationale → [decisions.md](./decisions.md#auto-deploy-guard-rails-check-live-status-not-a-stored-flag) and [decisions.md § skipped guard job cascades](./decisions.md#a-skipped-guard-job-cascades-to-skip-downstream-jobs-unless-their-if-says-otherwise).

**Teardown (`Client-HTML5-Undeploy.yml`):** manual, requires typing `UNDEPLOY`. `soft` (default) deploys a minimal "build offline" page (also carries the inert `CNAME` file), site stays registered. `hard` calls `DELETE /repos/{owner}/{repo}/pages`, so the URL 404s **and the custom domain setting below is lost** — must be re-added manually after the next deploy recreates the site. Redeploy = run the deploy workflow again. Hard undeploy may need repo-admin rights beyond the default `GITHUB_TOKEN` — the workflow reports a 403 explicitly and points at soft mode; **untested against the live repo**, which is why soft is the default. Does not touch the physical web backup itself — see below.

**Custom domain:** live at `https://play.tod.galaxxigames.com` — a Cloudflare `CNAME` record (`play.tod` → `trake25.github.io`, Proxied/orange-cloud) in the shared `galaxxigames.com` zone, same zone as the K3s and physical-backup hostnames ([deployment.md](./deployment.md)). Cloudflare's free Edge Certificate (Universal SSL) only covers the zone apex and *one* level of subdomain below it — `play.tod` is two levels deep and only works because a certificate for it already exists; every other hostname added since (`wsplaytod`, `playtod`, `devwstod1`, etc.) is deliberately kept exactly one level deep to stay inside that same free coverage. The domain itself is registered as a **manual, one-time repo setting** (Settings → Pages → Custom domain), not derived from the `CNAME` file the workflow writes into `build/web`/`build/offline` — that file only matters for the legacy Jekyll/branch-based Pages build, and this workflow uses the Actions-based method (`upload-pages-artifact` + `deploy-pages`), which ignores it. **Enforce HTTPS** must stay on: the Godot Web export checks `window.isSecureContext` and refuses to boot over plain HTTP. Why this can't be automated in the workflow → [decisions.md](./decisions.md#pages-custom-domain-must-be-set-manually).
...
**Independent of the K3s web servers and the physical backup:** this workflow's `play.tod.galaxxigames.com` (note the dot — a distinct hostname from K3s's one-level `playtod.galaxxigames.com`/`todtest.galaxxigames.com`) and the backup's `devtod1`/`devtod2.galaxxigames.com` ([deployment.md § Backup](./deployment.md#backup-physical-machine-4-dev-instances)) are independent hosts with no orchestration linking them.
