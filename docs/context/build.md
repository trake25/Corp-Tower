...
**Teardown (`Client-HTML5-Undeploy.yml`):** manual, requires typing `UNDEPLOY`. `soft` (default) deploys a minimal "build offline" page (also carries the inert `CNAME` file), site stays registered. `hard` calls `DELETE /repos/{owner}/{repo}/pages`, so the URL 404s **and the custom domain setting below is lost** — must be re-added manually after the next deploy recreates the site. Redeploy = run the deploy workflow again. Hard undeploy may need repo-admin rights beyond the default `GITHUB_TOKEN` — the workflow reports a 403 explicitly and points at soft mode; **untested against the live repo**, which is why soft is the default.

**Custom domain:** live at `https://play.tod.galaxxigames.com` — a Cloudflare `CNAME` record (`play.tod` → `trake25.github.io`, Proxied/orange-cloud) in the shared `galaxxigames.com` zone, same zone/pattern as `ws.tod.galaxxigames.com` and `devtod.galaxxigames.com` ([deployment.md](./deployment.md)); Cloudflare places no restriction on subdomain nesting depth. The domain itself is registered as a **manual, one-time repo setting** (Settings → Pages → Custom domain), not derived from the `CNAME` file the workflow writes into `build/web`/`build/offline` — that file only matters for the legacy Jekyll/branch-based Pages build, and this workflow uses the Actions-based method (`upload-pages-artifact` + `deploy-pages`), which ignores it. **Enforce HTTPS** must stay on: the Godot Web export checks `window.isSecureContext` and refuses to boot over plain HTTP. Why this can't be automated in the workflow → [decisions.md](./decisions.md#pages-custom-domain-must-be-set-manually).
