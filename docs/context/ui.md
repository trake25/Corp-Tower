...
| `InventoryController` | 3-slot inventory cards, drag-to-place input, **lane selection** (`lane_for_global_pos` buckets the release x within `TowerDropZone` into `left`/`center`/`right`, passed to `on_block_pressed(index, lane)` → `NetworkManager.place_block(index, lane)`), local placement cooldown, draw-pile preview shown in the always-visible [Team Inventory Panel](#team-inventory-panel) |
...
| **Tower Stack** | `Cor/Scripts/TowerStack.gd` | `set_tower(blocks, height, target_height, stability=100, diagnostics={})`, `set_player_color_map(map)`, `clear_tower()`, `scroll_offset_changed(pixels: float)` signal | Visual-only; server owns tower state. Renders the **5-column grid** (`GRID_WIDTH` 5, `GRID_CENTER_COL` 2.0 centers columns by `originX + cell_x − GRID_CENTER_COL`; previously the 7-wide `− 3.0`). No lane-guide overlay yet (planned — see [decisions.md](./decisions.md#five-fixed-bricks--3-lane-placement-replace-the-size-ramp)). Brick size is fixed at `BRICK_UNIT_SIZE` (34px design units) — bricks never shrink to fit, regardless of tower height; see [decisions.md](./decisions.md#fixed-brick-size--parallax-scroll-replaces-shrink-to-fit-tower-rendering). `_scroll_offset_units()` starts shifting the drawn tower up once `current_height` passes `SCROLL_TRIGGER_RATIO` (0.65) of the visible brick capacity — not only once the tower completely fills the draw area — ramping continuously from 0 with no jump at the trigger point, so the parallax visibly begins partway up the screen rather than only once bricks are about to hit the Top Indicator. No upper bound (unlimited pan). Does **not** set `clip_contents`; `_is_rect_visible()`'s bottom bound extends past the Control's own rect to the actual screen bottom (`get_parent().size.y - position.y`), so the oldest bricks keep rendering as they scroll down and are hidden only by later siblings drawing over them (the `ActionRow` inventory buttons, which start at `offset_top = 827`) rather than vanishing at the tower's own fixed boundary — see [decisions.md](./decisions.md#fixed-brick-size--parallax-scroll-replaces-shrink-to-fit-tower-rendering). Emits `scroll_offset_changed(pixels)` whenever the pixel scroll amount changes (deduped via `_last_scroll_pixels`) — consumed by [Background Parallax](#leaf-components) via [Main UI Controller](#main-ui-controller). Tilt has two layers: `tower_tilt_deg` (target lean, from `diagnostics.tiltAngleDeg`) and `displayed_tilt_deg` (eased via `TILT_EASE_SPEED`, glides toward target — smoothing only, the underlying tilt is fully server-recalculated on every placement, never animated server-side). Once `diagnostics.collapsed` is true, rendering exaggerates lean to a fixed `COLLAPSE_TILT_DEG` (70°) — well past the live-play cap (`towerMaxTiltAngleDeg`, typically 24°) — a purely cosmetic "sell the collapse" flourish for an already-ended level. Rotates around a bottom-center pivot (matches where it rests on the ground); visibility culling done in pre-rotation space as a cheap approximation, fine at live-play angles and the bounded post-collapse angle. |
| **Background Parallax** | `Cor/Scripts/BackgroundParallax.gd` | `set_scroll_pixels(pixels: float)`, `@export parallax_ratio: float`, `@export ease_speed: float` | Visual-only leaf script, attached to **two** separate `TextureRect`s in [Game UI Scene](#game-ui-scene) rather than one — `BgArt` (the sky, script default `parallax_ratio = 0.3`, no per-node override) and `PlayField/PlatformArt` (the ground platform, overridden to `parallax_ratio = 1.1` on that node). Each instance eases its own node's vertical position toward `pixels * parallax_ratio` via `ease_speed`-driven `lerpf` (both currently at the script default `4.0`), independently, since the ratio/speed were converted from module consts to `@export` vars specifically so each attached instance can be tuned separately from the Inspector. The sky moves slower (further away, classic background parallax layer); the platform moves *faster than the tower's own scroll* so it never lags behind and desyncs from the tower's base as bricks scroll away — see [decisions.md](./decisions.md#fixed-brick-size--parallax-scroll-replaces-shrink-to-fit-tower-rendering) for why the platform needed this at all (it's meant to recede with the background, not stay HUD-fixed). Panning `BgArt` down reveals the existing full-screen `Background` `Panel` behind it (solid sky-blue `Color(0.55, 0.83, 0.96, 1)` from the theme's panel style) above `BgArt`'s new position, standing in for extra sky as a placeholder — no new node was added for this fill. Both instances are driven by [Tower Stack](#leaf-components)'s `scroll_offset_changed` signal, wired in [Main UI Controller](#main-ui-controller). Asset implication: since a flat color has no edge, it trivially supports unlimited pan; a future real art replacement must be a seamlessly vertically-tileable sky/cloud texture (top edge matching bottom edge) to preserve that, delivered through the existing [Private Asset Pipeline](./build.md#private-asset-pipeline). |
