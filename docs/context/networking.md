...
- Endpoint is resolved at build time, not hardcoded: `NetworkManager.gd`'s `SERVER_URL`/`FAILOVER_SERVER_URL` alias `EndpointConfig.PRIMARY`/`FAILOVER`, generated into the committed-but-regenerated `Sys/NetMan/Endpoint_Config.gd` by `scripts/write-endpoint-config.sh` before each client build. Prod/test K3s builds ship an empty `FAILOVER` (no failover); the physical backup's two dev builds fail over to each other. Full per-build values → [build.md § Client endpoint config](./build.md#client-endpoint-config).
- Client: Godot `WebSocketPeer`, wrapped by [NetworkManager](#networkmanager), which tries the primary endpoint first and automatically falls over to the backup endpoint on connect failure/timeout — only when `FAILOVER_SERVER_URL` is non-empty — see [NetworkManager](#networkmanager) below.
...
| `place_block` | Valid room, player, state, cooldown, inventory, block index; `column` = integer target origin column (an absent/invalid column falls back to the level's site minimum). Server maps column → grid `originX` via `resolveColumnOriginX`, clamped to the brick's valid placeable range (`getPlaceableOriginRange`) for **that level's site**. Optional `originY` = the integer row the client aimed at; it becomes the brick's release row when legal, and the brick falls from there. Omitted (bots, unsnapped drags) or illegal by arrival → released above the tower instead, never rejected |
| `activate_power` | Valid room, player, held item at `slot`, shared activation cooldown. **No target field** — the effect is room-wide, caster included. Every power is this one message plus its catalog id; the broadcast `powerEvents[]` entry carries `meta.blocksAdded` for `replenish` so the toast can name the count |
...
- On every drag move, `InventoryController` offsets the pointer by the grip lift and asks `TowerStack.resolve_snap(cells, ghost_global_pos)`, which returns the whole resolution `{valid, snapped, exact, column, origin_y, target_point, matched_vertex}` — the closest *legal* pairing between one of the brick's own corners and one of the tower's snap points (platform boundaries + placed-brick corners), rejecting pairings whose footprint would leave the placeable column range (mirrors the server's `getPlaceableOriginRange` exactly) or overlap a placed cell, then applying the same gravity the server will. Release inside `TowerDropZone` sends `place_block` with the inventory index, that resolved `column`, and `origin_y` **only when `exact`** (a beyond-radius aim leaves the row to the server). Release elsewhere cancels locally with no server message.
...
| `originX` / `originY` | Resolved structural coordinates (column-derived `originX`, gravity-settled `originY`) on the `towerGridWidth` grid, confined to the level's placeable site |
| `accessibility` | The room's input-mode defaults from `Game_Config.accessibility` (currently `parallelPlacement`). Sent every tick; a player's local override wins over it |
| `visualHooks` | The room's `Game_Config.visualHooks` group: the `impactBeat`/`screenShake` toggles **and every [Impact Beat](./ui.md#leaf-components) duration**. Sent every tick, and the only route the durations take — they are not `debug_config` keys |
...
| `lastLevelSummary` | `result`, `reason`, `teamLevelScore`, `mvpId`, `mvpScore`, `exactFinish`, `overbuildHeight`, `finisherId`, `finishingBlock`, `carriedBlockCount`, `sideQuest` (the level's quest with its `claimedBy`/`rewardId`, for the summary's quest line), `players[]` (per-player: id, bot flag, level score, previous/final total, contributed height, MVP flag, bonus breakdown). Impact failures also include `impactScoreStatus` |
...
- **Methods:** `connect_server(is_auto_reconnect := false, is_failover_retry := false)`, `disconnect_server()`, `toggle_connection()`, `place_block(block_index, column := -1, origin_y := -1)` (sends `originY` only when `origin_y >= 0`), `send_quick_chat(slot)`, `activate_power(slot)`, `update_config(key, value)`.
...
- **Primary/backup failover:** every fresh connect (`is_auto_reconnect` and `is_failover_retry` both false) starts at `SERVER_URL` (primary). `WebSocketPeer` has no built-in connect timeout, so a manual one is enforced: a connection stuck in `STATE_CONNECTING` past `CONNECT_TIMEOUT_SECONDS` (`5.0`) is force-closed via `ws.close()`, which flows into the normal `STATE_CLOSED` handling. If that closure happened before the socket ever reached `STATE_OPEN`, the backup hasn't been tried yet this connect cycle (`tried_failover`), and `FAILOVER_SERVER_URL` is non-empty, NetworkManager retries once against it (`connect_server(false, true)`), emitting `"Primary server unreachable, trying backup..."`. A build with an empty `FAILOVER_SERVER_URL` (prod/test) never takes this branch — it just reports disconnected. Once failed over, in-game auto-reconnects (real `is_auto_reconnect` retries) keep targeting the backup rather than flipping back to primary — a fresh manual connect (`toggle_connection()`) is what resets to primary again.
