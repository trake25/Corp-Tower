...
**Responsibilities:** create bricks from the **5 fixed shapes** (`brickShapes`, weighted by `brickWeights`, all available from level 1 — no size unlock), applying a **random rotation** to the shape's canonical `cells` (internal `getRotations`/`rotateCellsCW`, deduped by cell layout) and a **random `anchorX`** within that rotation's column range (internal `getRandomAnchorX`) at creation — each created block is `{ id, shapeId, cells, anchorX, derived height }`; build/shuffle/deal the draw pile (sized via Game Config's generated-pile scaling); generate a **solvable** opening hand (retries until hand+pile can exactly reach target height, with enough precision blocks and surplus within bounds — falls back to the last attempt if none qualifies); refill a hand slot after placement; trim an oversized hand to `maxActiveBlocks` (keeping tallest/largest); generate a useful refresh set; prepare team carry-over blocks on completion.
