...
- Deals opening hands/draw piles through that engine, runs simple smart-play placement logic — picking the highest-stability **column** per brick via `chooseStablestColumn` (mirrors the bot heuristic, tries every valid column in `engine.getPlaceableOriginRange(block)`) — to simulate a level, prints CSV metrics: completion rate, exact-finish rate, overbuild, placement count, score spread. (Current caveat: the greedy bot builds narrow towers, so high-level completion reads low — a tuning signal, not a correctness failure; see [gameplay.md](./gameplay.md).)
