...
| **Lane** | One of three placeable columns — **left / center / right** = grid columns 3 / 4 / 5 on the 9-wide tower. Columns 0–2 and 6–8 are outer overflow only. The player picks a lane; the server maps it to `originX` via the brick's `anchorX`. |
| **`anchorX`** | The local cell column of a brick that lands on the chosen lane column, chosen **randomly** among the drawn rotation's columns at block creation (not a fixed per-shape value) and fixed for that block's lifetime regardless of which lane is later picked. `originX = laneColumn − anchorX`, clamped to the grid. Client shows a placeholder marker on this column so the player can see it before placing (see [ui.md](./ui.md#leaf-components)). |
