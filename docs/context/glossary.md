...
| **Shape ID** | One of the **5 fixed brick types**: `I`, `O`, `L`, `T`, `Z`, all 4-cell, available from level 1. `Game_Config.brickShapes` cells define only each shape's canonical (unrotated) layout — the dealt block gets a **random rotation** at generation (see [decisions.md](./decisions.md#random-rotation-9-column-lane-grid-and-per-instance-random-anchor)), so actual height/width per draw varies 1–4, not a fixed per-shape number. (Old size-1..6 variant ids like `I4H`/`I5V`/`J`/`S` are retired.) |
| **Lane** *(retired)* | Formerly one of three placeable columns (left/center/right = grid columns 3/4/5 on the 9-wide tower). Replaced by direct **column** placement on the 14-wide tower — see **Placeable column range** and **Snap point** below, and [decisions.md](./decisions.md#14-column-corner-snap-placement-replaces-lanes-and-anchors). |
...
| **Placeable column range** | Columns **4–9** (inclusive) of the 14-column tower grid (`towerGridWidth` = 14, `placeableColumnMin`/`placeableColumnMax`) — the only columns a brick may occupy. Columns 0–3 and 10–13 are permanently unplaceable: a **hard exclusion**, unlike the retired lane system's overflow columns — a brick's entire footprint must fit within 4–9. |
| **Snap point** | One of 7 fixed points along the platform, at each column boundary spanning the placeable range (boundaries of columns 4–9). While dragging, the brick's corner nearest the cursor snaps to the nearest snap point, clamped to keep the full footprint within the placeable range — this resolves the `column` sent to the server (`Game_Engine.resolveColumnOriginX`). Visualized with placeholder filled dots (platform/placed bricks) and hollow dots (the dragged ghost) — see [ui.md](./ui.md#leaf-components). |
