...
| **Impact Beat** | The end-of-level signature moment: the camera zooms out to reveal the whole tower, the verdict wave runs, the bars flash, and the camera returns — ~2s added to the level-summary wait. Configured under `Game_Config.visualHooks` ("visual hooks", the group and its two debug kill switches) and rendered client-side. See [ui.md](./ui.md#leaf-components). |
| **Verdict wave** | The bottom-to-top sweep inside an Impact Beat that flips each placed brick's face to its placer's Impact verdict — smiley if that player met their share this level, worried if not. Display only, cleared when the beat ends. |
