...
| **Snap point** | A lattice point a dragged brick's corner can dock to: the 7 fixed platform points (each column boundary spanning columns 4–9) **plus every true outline corner of every already-placed brick**, so the set grows as the tower is built. Resolution pairs each corner of the held brick against each snap point and takes the closest pair, which fixes the `column` sent to the server (`Game_Engine.resolveColumnOriginX`). Shown as light rings, with the active target enlarged and filled — see [ui.md](./ui.md#leaf-components). |
